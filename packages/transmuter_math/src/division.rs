...
use cosmwasm_std::{ensure, Decimal, Timestamp, Uint64};
...
impl Division {
    pub fn new(
        ...
        prev_value: Decimal,
            ...
            TransmuterMathError::NonMonotonicTime
        );
    ...
    ) -> Result<bool, TransmuterMathError> {
        ...
        let (mut processed_division, mut integral) = match first_division {
            ...
            None => {
                ...
                return match latest_removed_division {
                    Some(Division { latest_value, .. }) => Ok(latest_value),
                    None => Err(TransmuterMathError::MissingDataPoints),
                };
            }
        };
        ...
        match latest_removed_division {
            ...
            None => {
                ...
                if total_elapsed_time.is_zero() {
                    return Err(TransmuterMathError::UndefinedMovingAverage);
                }
    ...
    fn adjusted_integral(&self, window_stared_at: Uint64) -> Result<Decimal, TransmuterMathError> {
...
mod tests {
    ...
    fn test_new_compressed_division() {
        ...
        let err = Division::new(started_at, updated_at, value, prev_value).unwrap_err();

        assert_eq!(err, TransmuterMathError::NonMonotonicTime {});
    }
    ...
    fn test_average_empty_iter() {
        ...
        assert_eq!(
            average.unwrap_err(),
            TransmuterMathError::MissingDataPoints {}
        );
    }
    ...
    fn test_average_single_div_updated_at_start() {
        ...
        assert_eq!(err, TransmuterMathError::UndefinedMovingAverage);
        ...
        let divisions = vec![
            {
                ...
                let value = Decimal::percent(20);
                let prev_value = Decimal::percent(10);
                Division::new(started_at, updated_at, value, prev_value).unwrap()
            },
            {
    ...
    fn test_next_started_at() {
        ...
        let started_at = Timestamp::from_nanos(90);
        let compressed_sma_division = Division {
            started_at,
